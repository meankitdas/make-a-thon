...
const Stopwatch = () => {
  ...
  useEffect(() => {
    const handleTimerUpdate = ({ timeLeft: newTime, isRunning: newIsRunning }) => {
      setTimeLeft(newTime);
    ...
    socket.on("timerCompleted", handleTimerCompleted);

    fetchTimerStatus();

    return () => {
      socket.off("timerUpdate", handleTimerUpdate);
      socket.off("timerCompleted", handleTimerCompleted);
      socket.disconnect();
    };
  }, []);
  ...
  const fetchTimerStatus = async () => {
    try {
      const { data } = await api.get("/api/timer/status");
      const { isRunning: currentIsRunning, timeLeft: currentTime } = data;
          ...
          >
            <div
              className={`bg-gray-800/60 backdrop-blur-md rounded-xl p-6 border border-emerald-500/30 shadow-lg transition-transform duration-200 ${
                hoveredUnitIndex === index ? "scale-105 -translate-y-1" : ""
              }`}
            >
