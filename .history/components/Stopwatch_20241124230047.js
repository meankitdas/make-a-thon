...
import { Timer, PlayCircle, PauseCircle, RefreshCw, CheckCircle } from 'lucide-react';
...
const Stopwatch = () => {
  ...
  useEffect(() => {
    ...
    socket.on('timerUpdate', ({ timeLeft: newTime, isRunning: newIsRunning }) => {
      setTimeLeft(newTime);
  ...
  const fetchTimerStatus = async () => {
    try {
      ...
      const { isRunning: currentIsRunning, timeLeft: currentTime } = response.data;
  ...
  const handleStop = async () => {
    try {
      await axios.post(`${API_URL}/api/timer/stop`);
    } catch (err) {
  ...
  const handleReset = async () => {
    try {
      await axios.post(`${API_URL}/api/timer/reset`);
      setCompleted(false);
    } catch (err) {
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-3 gap-4 max-w-xl mx-auto mb-8">
        {[
          ...
          >
            <div className={`bg-gray-800/60 backdrop-blur-md rounded-xl p-6 border border-emerald-500/30 shadow-lg transition-transform duration-200 ${
